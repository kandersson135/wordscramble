$(document).ready(function() {
  const words = ['microsoft', 'apple', 'spotify', 'facebook', 'twitter', 'google', 'wikipedia', 'tiktok', 'instagram', 'netflix'];
  ...
  function checkWord() {
    ...
    if (userInput === currentWord) {
      ...
      if (currentLevel > words.length) {
        localStorage.setItem("ws-g6", 10);
        setTimeout(function() {
          window.location = "index.html";
        }, 800);
      } else {
        localStorage.setItem("ws-g6", currentLevel);
      }
