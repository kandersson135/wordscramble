$(document).ready(function() {
  const words = ['sverige', 'frankrike', 'brasilien', 'japan', 'sydafrika', 'kanada', 'australien', 'tyskland', 'indien', 'mexiko'];
  ...
  function checkWord() {
    ...
    if (userInput === currentWord) {
      ...
      if (currentLevel > words.length) {
        localStorage.setItem("ws-g15", 10);
        setTimeout(function() {
          window.location = "index.html";
        }, 800);
      } else {
        localStorage.setItem("ws-g15", currentLevel);
      }
